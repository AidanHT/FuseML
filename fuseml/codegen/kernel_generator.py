...
class TritonKernelGenerator:
    ...
    def _section_k_loop(
        ...
        return "\n".join([
            ...
            "        # Mask to prevent out-of-bounds reads along the K dimension",
            "        k_mask = offs_k < K - k * BLOCK_SIZE_K",
            "",
            f"        # Load (BLOCK_SIZE_M x BLOCK_SIZE_K) tile of {ln} from HBM → SRAM",
            f"        {ln} = tl.load({ln}_ptrs, mask=k_mask[None, :], other=0.0)",
            f"        # Load (BLOCK_SIZE_K x BLOCK_SIZE_N) tile of {rn} from HBM → SRAM",
            f"        {rn} = tl.load({rn}_ptrs, mask=k_mask[:, None], other=0.0)",
            "",
