...
class TestRunEndToEnd:
    ...
    def test_run_no_fusible_preserves_numerics(self):
        ...
        x = torch.randn(2, 64)
        gm = _trace_fn_no_grad(add_one, torch.randn(2, 64))
...
# ---------------------------------------------------------------------------
# Tests — graph surgery (_apply_surgery)
# ---------------------------------------------------------------------------
# These tests verify the graph rewriting phase: placeholder insertion, input
# wiring, output rewiring, dead code elimination, and metadata attachment.
# ---------------------------------------------------------------------------

def _run_surgery(gm: torch.fx.GraphModule) -> tuple[torch.fx.GraphModule, list[FusionGroup]]:
    """Run full fusion pass and return (modified gm, groups found)."""
